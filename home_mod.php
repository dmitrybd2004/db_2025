<?php
    ...
    if (!isset($_SESSION["username"])) {
        header("Location: login_page.php?error=" . urlencode("Access denied"));
        exit();
    }

    $name = $_SESSION["username"];
    $query = "SELECT * FROM roles WHERE roles.username='$name' AND roles.role='moderator'";
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="items-container">
                <?php
                    $query = "SELECT lot.item_id, lot.buyer_name, lot.item_name, lot.price, lot.image_name, refunds.request_id, refunds.report_type FROM 
                    lot JOIN refunds ON lot.item_id = refunds.item_id
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {                      
                            ...
                            echo "<h5> Request by: "  . htmlspecialchars($row['buyer_name']) . "</h5>";
                            echo "<form action='process.php' method='post'>";
                            echo "<input type='hidden' name='item_name' value='" . htmlspecialchars($row['item_name']) . "'>";

                            echo "<input type='hidden' name='request_id' value='" . $row['request_id'] . "'>";
                            // echo "<input type='hidden' name='item_id' value='" . $row['item_id'] . "'>";
                            // echo "<input type='hidden' name='request_type' value='" . $row['report_type'] . "'>";
                            echo "<button type='submit' class='btn btn-success' onclick='saveScrollPosition()'>Review request</button>";
                        ...
                        if($result->num_rows > $limit){
                            ...
                            echo "<div class='show-more-container'>";
                            echo "<button type='submit' class='btn btn-success' disabled>No more requests available</button>";
                            echo "</div>";
                        }
                    } else {
